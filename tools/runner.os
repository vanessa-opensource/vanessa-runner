...
Процедура ВывестиСправку()
	...
	Сообщить("		--pathxunit путь к внешней обработке, по умолчанию ищу в vendor/xunitfor1c/xddtestrunner.epf");
...
Процедура ДополнитьАргументыИзФайлаНастроек(Команда, ЗначенияПараметров, НастройкиИзФайла) 
...
Процедура ДополнитьСоответствиеСУчетомПриоритета(КоллекцияОсновная, КоллекцияДоп = Неопределено)
	...
	Для Каждого Элемент из КоллекцияДоп Цикл 
		...
		//КоллекцияОсновная.Получить(Элемент.Ключ);
		Если Значение = Неопределено Тогда 
...
Процедура ОсновнаяРабота()
	...
	Попытка
		...
		Если НастройкиИзФайла.Количество() > 0 Тогда 
			ДополнитьАргументыИзФайлаНастроек(Команда, Аргументы.ЗначенияПараметров, НастройкиИзФайла);
			Для каждого Элемент из Аргументы.ЗначенияПараметров Цикл 
				Лог.Отладка(Элемент.Ключ + ":"+Элемент.Значение);
			КонецЦикла;
		КонецЕсли;
		
		Если Команда = ВозможныеКоманды().ДекомпилироватьEpf Тогда
			Декомпилировать(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["--outDir"]), 
		...
		ИначеЕсли Команда = ВозможныеКоманды().РазобратьРасширение Тогда 
			РазобратьНаИсходникиРасшрение(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]), 
							Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
							Аргументы.ЗначенияПараметров["--v8version"]);
							
		ИначеЕсли Команда = ВозможныеКоманды().СобратьРасширение Тогда 
			СобратьИзИсходниковРасшрение(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]),
							Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
							Аргументы.ЗначенияПараметров["--v8version"]);
		...
		ИначеЕсли Команда = ВозможныеКоманды().ЮнитТест Тогда
			ЗапуститьТестироватьЮнит(ПолныйПуть(Аргументы.ЗначенияПараметров["inputPath"]), ПараметрыОтчетовXUnit(Аргументы.ЗначенияПараметров["--reportsxunit"], ПолныйПуть(Аргументы.ЗначенияПараметров["--reportxunit"])),
							Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
