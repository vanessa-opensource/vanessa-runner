...
Процедура СледитьЗаИзменениямиФайловВРабочемКаталоге(Значение, КэшПутей, Фильтр = "")
	...
	Для каждого Файл из СписокФайлов Цикл
		...
		ОтносительныйПутьФайла = ОбщиеМетоды.ОтносительныйПуть(КаталогВходящийДляРазбора, ИмяФайлаНазначения);
	...
	Для каждого Файл из СписокФайлов Цикл
		...
		Если Изменен = Истина Тогда
			...
			ИначеЕсли ОбработкуНашли = Истина Тогда
				...
				КаталогВыгрузкиОбработки = Новый Файл(ОбъединитьПути(КаталогИсходящий, ОбщиеМетоды.ОтносительныйПуть(ПолныйПуть(КаталогВходящий), ПапкаИсходников), ПутьОтносительно)).ПолноеИмя;
			...
			Иначе
				НовыйПутьВыгрузки = ОбъединитьПути(КаталогИсходящий, ОбщиеМетоды.ОтносительныйПуть(ПолныйПуть(КаталогВходящий), Файл.ПолноеИмя));
	...
	Попытка
		...
		ИначеЕсли Команда = ВозможныеКоманды().РазобратьРасширение Тогда 
			РазобратьНаИсходникиРасширение(
				Аргументы.ЗначенияПараметров["extensionName"], ПолныйПуть(Аргументы.ЗначенияПараметров["outputPath"]), 
				Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"],
				Аргументы.ЗначенияПараметров["--db-pwd"], Аргументы.ЗначенияПараметров["--v8version"]);
...
Функция УказанПараметрТолстыйКлиент(Знач ПараметрТолстыйКлиентИзКоманднойСтроки)
