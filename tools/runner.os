...
Процедура ЗапуститьОбновлениеКонфигурации(Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="",
                                        Знач КлючРазрешенияЗапуска = "", Знач ВерсияПлатформы="", Знач ДополнительныеКлючиЗапуска = "") 
...
Процедура Инициализация()
    ...
    Иначе
        ...
        мИмяФайлаИнформации = указательНаВременныйФайл.ИмяБезРасширения;
        Аппендер = Новый ВыводЛогаВФайл();
        Аппендер.ОткрытьФайл(ОбъединитьПути(КаталогЛогов, ИмяСкрипта()+"_"+мИмяФайлаИнформации+".log"));
        указательНаВременныйФайл = Неопределено;
