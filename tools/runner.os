...
Процедура СледитьЗаИзменениямиEPFToSrc(Значение, КэшПутей, Фильтр = "")
    ...
    Для каждого Файл из СписокФайлов Цикл
		...
		ОтносительныйПутьФайла = ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.ПолноеИмя);
		ПутьФайлаИсходников = ОбъединитьПути(КаталогИсходящий, ОтносительныйПутьФайла);
        ...
        // Если Изменен Тогда
            ...
            Если ЭтоБинарныйОбъект1С Тогда
				...
				НовыйПутьВыгрузки = ПутьФайлаИсходников;
				// НовыйПутьВыгрузки = ОбъединитьПути(КаталогИсходящий, ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.ПолноеИмя));
				// Лог.Отладка("   относительный путь :%1", ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.ПолноеИмя));
				...
				Если Изменен Тогда
					Лог.Отладка("Разбираем внешнюю обработку %1", Файл.ПолноеИмя);
					// Лог.Отладка("Разбираем внешнюю обработку %1", Файл.Путь);
					...
					Лог.Отладка("   новый путь         :%1", НовыйПутьВыгрузки);
            ...
            Иначе
				...
				Лог.Отладка("Анализ копирования простого файла %1", ПутьФайлаИсходников);
				ФайлНовый = Новый Файл(ПутьФайлаИсходников);
				...
				Если Изменен Тогда
					Лог.Отладка("Копирую простой файл");
					Лог.Отладка("   относительный путь:%1", ОтносительныйПутьФайла);
					Лог.Отладка("   источник          :%1", КаталогВходящийДляРазбора);
					Лог.Отладка("   назначение        :%1", КаталогИсходящий);
					Лог.Отладка("   новый путь        :%1", ПутьФайлаИсходников);
...
Процедура ОсновнаяРабота()
    ...
    Попытка
        ...
        ИначеЕсли Команда = ВозможныеКоманды().ЮнитТест Тогда
			ЗапуститьТестироватьЮнит(ПолныйПуть(Аргументы.ЗначенияПараметров["ПутьВходящихДанных"]), ПараметрыОтчетовXUnit(Аргументы.ЗначенияПараметров["--reportsxunit"], ПолныйПуть(Аргументы.ЗначенияПараметров["--reportxunit"])),
                            Аргументы.ЗначенияПараметров["--ibname"], Аргументы.ЗначенияПараметров["--db-user"], Аргументы.ЗначенияПараметров["--db-pwd"],
