...
Процедура СледитьЗаИзменениямиEPFToSrc(Значение, КэшПутей, Фильтр = "")
    ...
    Для каждого Файл из СписокФайлов Цикл
		...
		ОтносительныйПутьКаталогаФайла = ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.ПолноеИмя);
		ПутьФайлаИсходников = ОбъединитьПути(КаталогИсходящий, ОтносительныйПутьКаталогаФайла);

		ЭтоБинарныйОбъект1С = ТипФайлаПоддерживается(Файл);
		Если Не ЭтоБинарныйОбъект1С Тогда
			Лог.Отладка("Копирую простой файл");
			Лог.Отладка("   относительный путь:%1", ОтносительныйПутьКаталогаФайла);
        ...
        Если Изменен Тогда
            ...
            Если ЭтоБинарныйОбъект1С Тогда
                ...
                НовыйПутьВыгрузки = ОбъединитьПути(КаталогИсходящий, ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.Путь));
                Лог.Отладка("   относительный путь :"+ПолучитьОтносительныйПутьФайла(КаталогВходящийДляРазбора, Файл.Путь));
                Лог.Отладка("   источник           :"+КаталогВходящийДляРазбора);
                Лог.Отладка("   назначение         :"+КаталогИсходящий);
