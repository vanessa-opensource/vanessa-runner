...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, 
...
Процедура РазобратьКаталог(Знач ОбъектКаталога, Знач КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
	...
	ОтносительныйПутьКаталогаПоТекущемуКаталогу = ПолучитьОтносительныйПутьФайла(ТекущийКаталог(), ПутьКаталога);
	ОтносительныйПутьКаталогаВыгрузкиПоТекущемуКаталогу = ПолучитьОтносительныйПутьФайла(ТекущийКаталог(), КаталогВыгрузки);
...
Функция РазобратьФайлВнешняяОбработка(Знач Файл, Знач КаталогВыгрузки, Знач КаталогКорень = "", Знач ВыводитьСообщения,
	...
	ОтносительныйПуть = ПолучитьОтносительныйПутьФайла(ТекущийКаталог(), ПутьФайла);
	ОтносительныйПутьКаталога = ПолучитьОтносительныйПутьФайла(ТекущийКаталог(), КаталогВыгрузки);
		...
		Если ЭтоБинарныйОбъект1С Тогда
			...
			Если Изменен Тогда
				...
				КаталогРазобранный = РазобратьФайлВнешняяОбработка(Файл, КаталогИсходящий, КаталогВходящийДляРазбора, 
					Ложь, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы);
