...
Процедура ЯСобираюПакетВоВременномКаталоге() Экспорт
	...
	СтрокаЗапуска = ?(ЭтоWindows, "call ", "") + СтрШаблон("opm build %1 -out %2", ТекущийКаталог(), ПутьВременногоКаталога);
...
Процедура ЯУстанавливаюПакетИзФайлаСобранногоПакета() Экспорт
	...
	ИмяСобранногоПакета = БДД.ПолучитьИзКонтекста("ИмяСобранногоПакета");
	ПутьКаталогаУстановки = "oscript_modules";
	ПутьИсполнителя = ОбъединитьПути(ПутьКаталогаУстановки, ИмяСобранногоПакета, "testrunner.os");
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ЭтоWindows = Найти(ВРег(СистемнаяИнформация.ВерсияОС), "WINDOWS") > 0;
	СтрокаЗапуска = ?(ЭтоWindows, "call ", "") + СтрШаблон("opm install -f %1 -l", ФайлСобранногоПакета.Имя);
	КодВозврата = ВыполнитьКоманду(СтрокаЗапуска);
