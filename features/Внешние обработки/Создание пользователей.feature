...
Сценарий: Создание первого пользователя-администратора
    ...
    # Когда Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/main.os run --command "ЗапуститьОбновлениеИнформационнойБазы;ЗавершитьРаботуСистемы;" --execute $runnerRoot\epf\ЗакрытьПредприятие.epf --ibconnection /Fbuild/ib --no-wait --language ru"
    Когда Я добавляю параметр "<КаталогПроекта>/src/main.os run" для команды "oscript"
    # И Я добавляю параметр '--command "СоздатьАдминистратора;Администратор"' для команды "oscript"
    И Я добавляю параметр "--command" для команды "oscript"
    # И Я добавляю параметр "СоздатьАдминистратора;Имя=Администратор" для команды "oscript"
