...
Сценарий: Инициализация рабочей базы по умолчанию с uk языком в ./build/ib
    Когда Я сохраняю каталог проекта в контекст
    Когда Я выполняю команду "oscript" c параметрами "<КаталогПроекта>/src/main.os init-dev --src ./cf --nocacheuse --language uk"
    И Я сообщаю вывод команды "oscript"
    Тогда Вывод команды "oscript" содержит "Оновлення конфігурації бази даних успішно завершено"
    И Код возврата команды "oscript" равен 0
    И Файл "build/ib/1Cv8.1CD" существует
    И Файл "build/ibservice/1Cv8.1CD" не существует
