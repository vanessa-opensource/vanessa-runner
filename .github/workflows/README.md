...
## Кэши платформы и EDT

Платформа 1С и 1С:EDT ставятся действием `1CDevFlow/onec-setup-action` с `cache: true`:
установленный каталог целиком лежит в кэше GitHub Actions под ключом `setup--<тип>--<версия>--<ОС>`.
GitHub удаляет кэш, к которому не обращались 7 дней. После этого первый же прогон скачивает
дистрибутив с releases.1c.ru, и если сайт или учётная запись файлы не отдают, падает шаг
установки — в прогоне, который к причине отношения не имеет.

`cache-keepalive.yml` по расписанию раз в три дня обращается к каждому кэшу и тем продлевает
срок хранения, а пропавший кэш создаёт заново. Его падение значит, что скачивание сломано, а кэша
уже нет: чинить нужно до того, как истекут остальные. Версии в матрице этого workflow повторяют
версии тестовых workflow — при смене версии меняются оба места.
