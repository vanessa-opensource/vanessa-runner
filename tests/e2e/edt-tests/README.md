...
| `validate/ТестValidateEdt.os`| `validate edt`                 | проверка EDT-проекта (1cedtcli validate), JUnit + исходный отчёт EDT (`--report`), порог `--min-severity`, XML-выгрузка (автоконвертация в EDT) |
