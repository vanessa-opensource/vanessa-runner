...
Процедура ОжидаемЧтоВыводСодержитРегулярноеВыражение(ШаблонРегулярногоВыражения) Экспорт

	РегулярноеВыражение = Новый РегулярноеВыражение(ШаблонРегулярногоВыражения);
	Совпадения = РегулярноеВыражение.НайтиСовпадения(СпособВывода.ВыводЛога());
	
	Ожидаем.Что(Совпадения, "Вывод должен содержать " + ШаблонРегулярногоВыражения).ИмеетДлину(1);
