...
::: tip
Позиционный аргумент `testsPath` из конфига 2.x переносится в `autumn-properties.json` ключом `testspath` в секции `vrunner.test.xunit` — скрипт миграции делает это автоматически. Путь из командной строки или переменной окружения `VRUNNER_TESTSPATH` имеет приоритет над файлом настроек.
:::
