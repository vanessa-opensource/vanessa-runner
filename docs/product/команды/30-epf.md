...
Внешние обработки не являются EDT-проектом, поэтому `epf compile`/`epf decompile` работают только с XML-исходниками. При обнаружении каталога EDT (или явном `--src-format edt`) команда завершится ошибкой. См. [Исходники в формате 1С:EDT →](./06-edt).
:::
