...
Собирает внешние обработки (`.epf`) и внешние отчёты (`.erf`) из XML-исходников.
Поддерживает обработку целого каталога с рекурсивным поиском.

Тип объекта определяется по корневому тегу XML, он же задаёт расширение результата:
`ExternalDataProcessor` → `.epf`, `ExternalReport` → `.erf`. В одном дереве исходников
обработки и отчёты могут лежать вперемешку — каждый объект соберётся со своим расширением.
...
| Аргумент | Описание |
|----------|----------|
| `SRC` | Каталог с XML-исходниками обработок и отчётов (по умолчанию - текущий каталог; в файле настроек — ключ `src`) |
...
| `--out` | - | Каталог для сохранения собранных обработок и отчётов |
...
`compile` сначала выгружает его в XML через `1cedtcli` (`export`), затем собирает
`.epf`/`.erf` из полученных XML. Требуется установленная 1С:EDT.
:::
...
Разбирает файл внешней обработки `.epf` или внешнего отчёта `.erf` (либо каталог с такими
файлами) в XML-исходники. При обходе каталога подхватываются оба расширения.
...
| Аргумент | Описание |
|----------|----------|
| `SRC` | Путь к EPF/ERF-файлу или каталогу с ними (**обязательный**, в файле настроек — ключ `src`) |

### Опции

| Опция | Переменная окружения | Описание |
|-------|---------------------|----------|
| `--R`, `--recursive` | - | Рекурсивный поиск EPF/ERF-файлов (для каталога) |
