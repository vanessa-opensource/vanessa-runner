...
Свойства расширения (`--safe-mode`, `--unsafe-action-protection`, `--active`) применяются
**отдельным шагом после загрузки**: в режиме `ibcmd` — средствами `ibcmd`, в режиме Конфигуратора —
обработкой в режиме 1С:Предприятие. Свойства `--used-in-rib`, `--scope-infobase` поддерживаются
только при `--ibcmd`.

В режиме Конфигуратора безопасный режим и защита от опасных действий по умолчанию **выключены**:
шаг установки свойств выполняется при каждой загрузке (запуск 1С:Предприятие), чтобы гарантировать
эти значения, даже если флаги не переданы. Флаги `--safe-mode` / `--unsafe-action-protection`
включают соответствующее свойство.
...
| `--update-db` | - | После загрузки обновить конфигурацию БД для расширения (UpdateDBCfg) |
