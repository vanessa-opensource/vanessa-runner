...
> **Формат исходников.** `cfe compile`, `cfe load` принимают каталог исходников расширения как в XML-дампе Конфигуратора, так и в формате **1С:EDT**; `cfe decompile` умеет выгружать в формат EDT. Опции `--src-format`, `--edt-version`, `--edt-workspace`. Подробнее: [Исходники в формате 1С:EDT →](./06-edt).
