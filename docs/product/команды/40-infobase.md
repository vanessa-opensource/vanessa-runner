...
|-------|---------------------|----------|
| `--src` | `VRUNNER_SOURCE` | Источник конфигурации: каталог исходников (XML-дамп или [1С:EDT](./edt)), `.cf` или `.dt` файл. Псевдоним (для обратной совместимости): `--source` |
| `--ext` | - | Расширение для загрузки: `.cfe`-файл или каталог XML/[EDT](./edt)-исходников. Можно указать несколько раз. Имя расширения определяется из имени файла/каталога |
| `--src-format` | - | Формат каталога исходников (применяется и к `--src`, и к `--ext`): `auto` (по умолчанию), `edt`, `xml`. См. [1С:EDT](./edt) |
...
4. Загрузка конфигурации из `--src` (если задан)
5. Загрузка расширений из `--ext` (если заданы) и применение их к БД
...
# Создать ИБ, загрузить конфигурацию и несколько расширений
# (.cfe-файл и каталог исходников; имена берутся из имени файла/каталога)
vrunner infobase init \
  --src ./src \
...
|-------|-------------|----------|
| `--src` | - | Источник конфигурации: каталог исходников или `.cf` файл. Псевдоним (для обратной совместимости): `--source` |
...
| Аргумент | Описание |
|----------|----------|
| `OUT` | Путь к создаваемому DT-файлу (**обязательный**, в файле настроек — ключ `out`) |
...
| Аргумент | Описание |
|----------|----------|
| `IN` | Путь к DT-файлу для восстановления (**обязательный**, в файле настроек — ключ `in`) |
