...
|-------|---------------------|----------|
| `--source` | `VRUNNER_SOURCE` | Источник конфигурации: каталог исходников (XML-дамп или [1С:EDT](./06-edt)), `.cf` или `.dt` файл |
| `--src-format` | - | Формат каталога исходников: `auto` (по умолчанию), `edt`, `xml`. См. [1С:EDT](./06-edt) |
