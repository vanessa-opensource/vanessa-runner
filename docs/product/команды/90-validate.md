...
Источник `--src` - EDT-проект, XML-выгрузка Конфигуратора либо файл конфигурации `*.cf` или расширения `*.cfe`. XML-выгрузка перед проверкой конвертируется во временный EDT-проект (`1cedtcli import`). Файл `*.cf`/`*.cfe` сначала разбирается в XML во временной файловой ИБ (Конфигуратором или `ibcmd`, как `cf decompile`/`cfe decompile`), затем конвертируется - это занимает минуты. Имя проекта в отчёте - имя каталога исходников или файла без расширения; для `*.cfe` оно же используется как имя расширения.
...
| Опция | Переменная окружения | Описание |
|-------|---------------------|----------|
| `--src`, `-s` | `VRUNNER_SRC` | Каталог EDT-проекта или XML-выгрузки Конфигуратора либо файл `*.cf`/`*.cfe` (по умолчанию - текущий) |
...
> Общие опции: [формат исходников](./common-options#формат-исходников), [EDT (`--edt-*`)](./edt), [отчёты](./reports), [файл настроек](./common-options#файл-настроек); [Конфигуратор или ibcmd](./common-options#конфигуратор-или-ibcmd), [платформа](./common-options#платформа) - для файлов `*.cf`/`*.cfe`.
...
### Формат исходников (--src-format)

| Значение | Поведение |
|----------|-----------|
| `auto` | EDT-проект (в том числе единственный вложенный) проверяется как есть, XML-выгрузка конвертируется (по умолчанию) |
| `edt` | каталог проверяется как есть; на XML-выгрузке - ошибка |
| `xml` | каталог конвертируется; если XML-выгрузки в нём нет - ошибка |

Для файлов `*.cf`/`*.cfe` опция `--src-format` не учитывается.

Конвертация большой конфигурации занимает заметное время (см. `--edt-timeout`, `--edt-vmargs`). Исходники расширения проверяются без проекта основной конфигурации - замечания по заимствованным объектам могут быть ложными.
...
# XML-выгрузка Конфигуратора - конвертируется в EDT-проект автоматически
vrunner validate edt --src ./cf --report-format junit --report-path ./build/reports/edt.xml

# Файл конфигурации - разбирается во временной ИБ через ibcmd и конвертируется в EDT-проект
vrunner validate edt --src ./build/1cv8.cf --ibcmd --min-severity none --report-format junit --report-path ./build/reports/edt.xml
```
