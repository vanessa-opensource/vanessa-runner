...
|----------|-----------------|
| _не указано_ | всё: основная конфигурация и все расширения (поведение по умолчанию) |
| `main` | только основная конфигурация |
| `AllExtensions` | все расширения базы, одним запуском |
| `<имя расширения>` | только это расширение |

```bash
# конфигурация и все расширения (по умолчанию) - два запуска конфигуратора
vrunner validate syntax-check

# только конфигурация
vrunner validate syntax-check --target main
...
::: warning Стоимость
Каждая цель — это отдельный запуск конфигуратора, то есть отдельная полная проверка. Проверка по умолчанию (конфигурация + все расширения) на больших конфигурациях займёт вдвое больше времени, чем `--target main`.
:::
