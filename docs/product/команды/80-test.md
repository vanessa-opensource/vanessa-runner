...
| `--report` | `VRUNNER_YAXUNIT_REPORT` | Путь к файлу или каталогу отчёта (`reportPath`) |
...
| `--workspace` | `VRUNNER_WORKSPACE` | Базовый каталог проекта для разрешения относительных путей (`workspacePath`) |
