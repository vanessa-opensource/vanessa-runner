...
| Переменная | Опция | Описание |
|------------|-------|----------|
| `VRUNNER_REPORT_PATH` | `--report-path` | Куда выгрузить отчёт о результатах ([подробнее](../команды/reports)) |
| `VRUNNER_JUNITPATH` | `--junitpath` | _(устарела)_ Путь к JUnit-отчёту команд `validate` |
| `VRUNNER_ALLURE_RESULTS` | `--allure-results` | _(устарела)_ Каталог результатов Allure команд `validate` |
| `VRUNNER_REPORTSXUNIT` | `--reportsxunit` | _(устарела)_ Отчёты `test xunit` в формате `Формат{Путь}` |
| `VRUNNER_YAXUNIT_REPORT` | `--report` | _(устарела)_ Путь к отчёту `test yaxunit` |
| `VRUNNER_EDT_REPORT` | `--report` | _(устарела)_ Файл результатов `1cedtcli validate` в исходном формате EDT |

Формат отчёта переменной окружения не имеет: опция `--report-format` повторяемая, как `--mode` и `--target`.
