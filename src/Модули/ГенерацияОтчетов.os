...
//   КаталогОтчетаВФорматеJUnitxml - Строка - Каталог сохранения отчета
//   РасширениеНабора - Строка - Базовая часть набора тестов
//
Процедура СформироватьОтчетВФорматеJUnit(ОшибокНет, РезультатТестирования, КаталогОтчетаВФорматеJUnitxml, РасширениеНабора = "") Экспорт
	
	ОбеспечитьПустойКаталог(КаталогОтчетаВФорматеJUnitxml);
	...
	ЗаписьXML = Новый ЗаписьXML;
	ИмяФайлаОтчета = ОбъединитьПути(КаталогОтчетаВФорматеJUnitxml, "" + (Новый УникальныйИдентификатор()) + "-syntaxCheck.xml");
	ЗаписьXML.ОткрытьФайл(ИмяФайлаОтчета);
...
Процедура СформироватьОтчетВФорматеAllure(ОшибокНет, ДатаНачала, РезультатТестирования, КаталогОтчетовAllure, РасширениеНабора = "") Экспорт
	...
	ОбеспечитьПустойКаталог(КаталогОтчетовAllure);
...
Процедура ОбеспечитьПустойКаталог(ПутьККаталогу)
