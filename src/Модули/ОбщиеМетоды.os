...
Процедура ДополнитьСоответствиеСУчетомПриоритета(КоллекцияОсновная, Знач КоллекцияДоп) Экспорт
	...
	Для Каждого Элемент Из КоллекцияДоп Цикл
		Лог.Отладка("Обрабатываю параметр:" + Элемент.Ключ);
		ЗначениеПодстановки = Элемент.Значение;
		Лог.Отладка("Значение параметра:" + ЗначениеПодстановки + " Тип:" + ТипЗнч(ЗначениеПодстановки));
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("Булево") И НЕ Значение Тогда
			Если ТипЗнч(ЗначениеПодстановки) = Тип("Строка") Тогда
				Если Элемент.Значение = "1" ИЛИ Нрег(ЗначениеПодстановки) = "истина" ИЛИ Нрег(ЗначениеПодстановки) = "true" Тогда
...
Функция ПодставитьЗначенияПеременныхСреды(СтрокаПодстановки)
	...
	Для каждого Совпадение Из Совпадения Цикл
			ЗначениеПеременнойСреды = ПолучитьПеременнуюСреды(Совпадение.значение);
			Если Не ЗначениеЗаполнено(ЗначениеПеременнойСреды) Тогда
				Лог.Отладка("Не удалось получить значение переменной среды: "+Совпадение.Значение);
				Продолжить;
			КонецЕсли;
		РВЗамены = Новый РегулярноеВыражение("\${" + Совпадение.значение + "}");
		СтрокаРезультат = РВЗамены.Заменить(СтрокаРезультат, ЗначениеПеременнойСреды);
		Лог.Отладка("Значение переменной среды """ + Совпадение.значение + """изменено на """ + ЗначениеПеременнойСреды + """");
	КонецЦикла;
