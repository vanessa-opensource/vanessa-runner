...
Процедура ДополнитьСоответствиеСУчетомПриоритета(КоллекцияОсновная, Знач КоллекцияДоп) Экспорт
	...
	Для Каждого Элемент Из КоллекцияДоп Цикл
		...
		Лог.Отладка("Значение параметра: %2 Тип: %1", ЗначениеПодстановки, ТипЗнч(ЗначениеПодстановки));
...
Функция ПодставитьЗначенияПеременныхСреды(СтрокаПодстановки)
	...
	Для каждого Совпадение Из Совпадения Цикл
			...
			Если Не ЗначениеЗаполнено(ЗначениеПеременнойСреды) Тогда
				Лог.Отладка("Не удалось получить значение переменной среды: %1", Совпадение.Значение);
				Продолжить;
			КонецЕсли;
			
		СтрЗаменить(СтрокаРезультат, "${" + Совпадение.значение + "}", ЗначениеПеременнойСреды);
		Лог.Отладка("Значение переменной среды ""%1"" изменено на ""%2""", Совпадение.значение, ЗначениеПеременнойСреды);
	КонецЦикла;
