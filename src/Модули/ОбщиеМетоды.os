...
Функция ИмяИсполняемогоФайлаПоУмолчанию(БазовоеИмя) Экспорт
	Возврат БазовоеИмя + ?(ПараметрыСистемы.ЭтоWindows, ".exe", "");
КонецФункции

Функция ПолучитьПутьКExeФайлуПлатформы(
		КаталогПлатформы, ВерсияПлатформы, ИмяФайла, ИмяФайлаПоУмолчанию,
		ИмяПараметра, КонтекстОшибки = "Инициализация") Экспорт

	Если НЕ ЗначениеЗаполнено(КаталогПлатформы) Тогда
		Возврат "";
	КонецЕсли;

	Ожидаем.Что(ЗначениеЗаполнено(ВерсияПлатформы),
		СтрШаблон("%1: при указании --v8-platform-path необходимо также указать --v8version", КонтекстОшибки)).ЭтоИстина();

	КаталогПлатформы = ПолныйПуть(КаталогПлатформы);
	ИмяФайла = ?(ЗначениеЗаполнено(ИмяФайла), ИмяФайла, ИмяФайлаПоУмолчанию);

	КандидатПуть = ОбъединитьПути(КаталогПлатформы, ВерсияПлатформы, "bin", ИмяФайла);
	ФайлКандидат = Новый Файл(КандидатПуть);
	Если ФайлКандидат.Существует() Тогда
		Возврат ФайлКандидат.ПолноеИмя;
	КонецЕсли;

	ВызватьИсключение СтрШаблон(
		"%1: не найден исполняемый файл платформы для параметра %2 по шаблону <%3>.",
		КонтекстОшибки, ИмяПараметра, КандидатПуть);
