...
Процедура ДобавитьБлокIbcmd(Знач Парсер, ОписаниеКоманды) Экспорт
	...
	ДобавитьПараметрыПодключенияСУБДIbcmd(Парсер, ОписаниеКоманды);

КонецПроцедуры

Процедура ДобавитьПараметрыПодключенияСУБДIbcmd(Знач Парсер, ОписаниеКоманды) Экспорт

	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--dbms-type", 
		"Тип СУБД. Разрешенные значения: MSSQLServer, PostgreSQL, IBMDB2, OracleDatabase 
