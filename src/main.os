...
Процедура ДополнитьЗначенияПараметров(Знач Команда, ЗначенияПараметров)
	...
	Иначе
		ЗначениеПараметраФайлНастроек = ОбщиеМетоды.ПолныйПуть(ОбщиеМетоды.ИмяФайлаНастроек());
		ФайлОбщихНастроек = Новый Файл(ЗначениеПараметраФайлНастроек);
		Если Не ФайлОбщихНастроек.Существует() Тогда
			ЗначениеПараметраФайлНастроек = Неопределено;
		КонецЕсли;
	КонецЕсли;
