...
Функция ВыполнениеКоманды()
	...
	ЗначенияПараметровОсновные = Новый Соответствие;
	

	ДополнитьЗначенияПараметров(Команда, ЗначенияПараметровОсновные, ЗначенияПараметров);
	
	Возврат МенеджерКомандПриложения.ВыполнитьКоманду(Команда, ЗначенияПараметров);
	
КонецФункции // ВыполнениеКоманды()

Процедура ДополнитьЗначенияПараметров(Знач Команда, ЗначенияПараметров, ЗначенияПараметровАргументы)
	
	ПараметрыСистемы.КорневойПутьПроекта = УстановитьКаталогТекущегоПроекта(ЗначенияПараметровАргументы["--root"]);
	...
	ОбщиеМетоды.ДополнитьСоответствиеСУчетомПриоритета(ЗначенияПараметровАргументы, ЗначенияПараметров);

	Если ЗначениеЗаполнено(ЗначенияПараметровАргументы["--ibname"]) Тогда
		ЗначенияПараметровАргументы.Вставить("--ibname", 
						ОбщиеМетоды.ПереопределитьПолныйПутьВСтрокеПодключения(ЗначенияПараметровАргументы["--ibname"]));
	КонецЕсли;
