...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды, "--groupbymetadata", "Группировать проверки в junit по метаданным конфигурации");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--exceptions", "Пропустить ошибки, указанные в файле исключений");
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИмяФайлаИсключенийОшибок = ПараметрыКоманды["--exceptions"];
...
Процедура ВывестиОтчетПроверкиКонфигурацииВФорматеJUnitXML(Знач ПутьОтчетаВФорматеJUnitxml, 
	...
	Если НетОшибок Тогда
		...
		ЗаписьXML = НачатьЗаписьОтчета(ПредставлениеНабораТестов, 1, 0, 0, ДатаНачала, "passed");
	...
	Иначе
		...
		Если ТестовыеСлучаи.Ошибки.Количество() = 0 Тогда
			...
			ЗаписьXML = НачатьЗаписьОтчета(ПредставлениеНабораТестов, 1, 0, ТестовыеСлучаи.Пропуски.Количество(), ДатаНачала, "passed");
		...
		Иначе
			...
			ТестовыеСлучаи.Ошибки.Количество() + ТестовыеСлучаи.Пропуски.Количество(),
...
Функция СодержимоеФайлаИсключенийОшибок(Знач ИмяФайлаИсключенийОшибок)
	...
	Пока ПрочитаннаяСтрока <> Неопределено Цикл
		Результат.Добавить(СокрЛП(НРег(ПрочитаннаяСтрока)));
		Лог.Отладка("Добавлено в исключения: %1", ПрочитаннаяСтрока);	
		ПрочитаннаяСтрока = ЧтениеТекста.ПрочитатьСтроку();
	КонецЦикла;
...
Функция НачатьЗаписьОтчета(Знач ПредставлениеНабораТестов, Знач КоличествоТестов, Знач КоличествоОшибок,
	Знач КоличествоПропусков, Знач ДатаНачала, Знач Статус)
...
Функция СледуетПропуститьОшибку(Знач ТекстОшибки, Знач ИсключенияОшибок)
	...
	Если ИсключенияОшибок.Найти(СокрЛП(НРег(ТекстОшибки))) = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
