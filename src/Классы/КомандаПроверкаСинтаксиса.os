...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды, "--groupbymetadata", "Группировать проеврки в junit 
...
Функция СтруктурироватьЛог(Знач ЛогПроверкиИзКонфигуратора, Знач ГруппироватьПоМетаданным = Ложь)
	...
	Если ГруппироватьПоМетаданным Тогда
		Для Каждого ТекСтрока Из СтрРазделить(ОбработанныйЛог, Символы.ПС) Цикл
			...
			Если ИмяТестСценария = Неопределено Тогда
				ИмяТестСценария = ИмяТестСценарияПредыдущаяСтрока;
			Иначе
				ИмяТестСценарияПредыдущаяСтрока = ИмяТестСценария;
			КонецЕсли;
