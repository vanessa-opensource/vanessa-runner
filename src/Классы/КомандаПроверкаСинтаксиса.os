...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	СохранятьОтчетВФайл = ЗначениеЗаполнено(ПутьОтчетаВФорматеJUnitxml) ИЛИ ЗначениеЗаполнено(ПутьОтчетаВФорматеAllure);
...
Функция ОбработатьЛогОшибок(ДатаНачала, ЛогПроверкиИзКонфигуратора, ГруппироватьПоМетаданным, ИмяФайлаИсключенийОшибок)
	...
	Для Ит = 0 По МассивСтрокОшибок.Количество() - 1 Цикл
		...
		Если СтрНачинаетсяС(ТекСтрока, "{") Тогда
			ТекСтрока = ТекСтрока + ": " + СокрЛП(МассивСтрокОшибок[Ит + 1]);
			Ит = Ит + 1;
		КонецЕсли;
