...
//
Функция ПолучитьПутьКRAC() Экспорт

	ТекущийПуть = ПараметрыКоманды["--rac"];
	ВерсияПлатформы = ПараметрыКоманды["--v8version"];
	РазрядностьПлатформы = ОбщиеМетоды.РазрядностьПлатформы(ПараметрыКоманды["--bitness"]);
	...
	ПутьКПлатформе = Конфигуратор.ПолучитьПутьКВерсииПлатформы(ВерсияПлатформы, РазрядностьПлатформы);
