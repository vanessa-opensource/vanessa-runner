...
Процедура Разобрать(Знач Путь, Знач КаталогВыгрузки,
	...
	НастройкиИзФайла = ОбщиеМетоды.ПрочитатьНастройкиФайлJSON(ПараметрыСистемы.КорневойПутьПроекта, "./build/cache.json", "./build/cache.json");
	...
	Если ЕстьИзмененияДляЗаписи Тогда
		ПутьККэшу =  Новый Файл(ОбъединитьПути(ПараметрыСистемы.КорневойПутьПроекта, "./build/cache.json")).Путь;
		Если Новый Файл(ПутьККэшу).Существует() Тогда
			ЗаписатьФайлJSON(ОбъединитьПути(ПараметрыСистемы.КорневойПутьПроекта, "./build/cache.json"), НастройкиИзФайла);
		КонецЕсли;
	КонецЕсли;
...
Процедура Собрать(Знач Путь, Знач КаталогВыгрузки, Знач СтрокаПодключения = "", Знач Пользователь = "",
	...
	Если ЕстьИзмененияДляЗаписи И НЕ ТолькоКопировать Тогда
		ПутьККэшу =  Новый Файл(ОбъединитьПути(ПараметрыСистемы.КорневойПутьПроекта, "./build/cache.json")).Путь;
		Если Новый Файл(ПутьККэшу).Существует() Тогда
			Лог.Отладка("Начали записывать");
			ЗаписатьФайлJSON(ОбъединитьПути(ПараметрыСистемы.КорневойПутьПроекта, "./build/cache.json"), НастройкиИзФайла);
			Лог.Отладка("закончили записывать");
		КонецЕсли;
	КонецЕсли;
...
Процедура СледитьЗаИзменениямиФайловВРабочемКаталоге(Значение, КэшПутей, Фильтр = "", ЕстьИзменения = Ложь)
	...
	Для каждого Файл из СписокФайлов Цикл
		...
		Если НЕ ЭтоБинарныйОбъект1С И НЕ РежимОдногоКаталога Тогда
			...
			Продолжить;
		ИначеЕсли НЕ ЭтоБинарныйОбъект1С И РежимОдногоКаталога Тогда
			Продолжить;
		КонецЕсли;
		...
		КаталогРазобранный = РазобратьФайлВнешняяОбработка(Файл, КаталогИсходящий, КаталогВходящийДляРазбора,
...
Процедура СледитьЗаИзменениямиФайловВРепозиторииИсходников(Значение, КэшПутей, Фильтр = "", ЕстьИзменения = Ложь)
	...
	Для каждого Файл из СписокФайлов Цикл
		...
		Если ТипЗнч(ДанныеФайла) <> Тип("Соответствие") Тогда
			...
			ДанныеФайла.Вставить("Хеш", ""); ДанныеФайла.Вставить("Время", "");
		КонецЕсли;
		...
		Если Изменен = Истина Тогда
			...
			ИначеЕсли ОбработкуНашли = Истина Тогда
				ПутьОтносительно = ?(ОбщиеМетоды.ПолныйПуть(КаталогВходящий) = ПапкаИсходников, "./", "../");
			...
			ИначеЕсли НЕ НеобходимоПрервать Тогда
				...
				ДанныеФайлаСобраный = ХэшФайла(ФайлСобранный);
				Кэш.Вставить(ФайлСобранный.ПолноеИмя,ДанныеФайлаСобраный);
				КэшОбновляемый.Вставить(ФайлСобранный.ПолноеИмя, ДанныеФайлаСобраный);
			КонецЕсли;
			Кэш.Вставить(Файл.ПолноеИмя, ХэшФайла(Файл));
			СписокОбработанных.Вставить(Файл.ПолноеИмя, Истина);
		КонецЕсли;
...
Процедура РазобратьКаталог(Знач ОбъектКаталога, Знач КаталогВыгрузки, Знач КаталогКорень,
	...
	Для Каждого Файл из Файлы Цикл
		...
		Иначе
			...
			ОтносительныйПутьКаталогаФайла = ФС.ОтносительныйПуть(КаталогКорень, Файл.ПолноеИмя);
...
Функция РазобратьФайлВнешняяОбработка(Знач Файл, Знач КаталогВыгрузки, Знач КаталогКорень = "",
			Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
