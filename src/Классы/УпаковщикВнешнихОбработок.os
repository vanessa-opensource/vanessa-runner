...
Процедура Разобрать(Знач Путь, Знач КаталогВыгрузки,
	...
	НастройкиИзФайла = НормализоватьНастройкиКэша(
		ОбщиеМетоды.ПрочитатьНастройкиФайлJSON(ПараметрыСистемы.КорневойПутьПроекта, ИмяКешФайла(), ИмяКешФайла()),
		КлючКэша);
...
Функция НормализоватьНастройкиКэша(Знач НастройкиИзФайла, Знач КлючКэша)
	Если ТипЗнч(НастройкиИзФайла) <> Тип("Соответствие") Тогда
		Лог.Отладка("Кэш в ./build/cache.json имеет некорректный тип, будет создан заново");
		НастройкиИзФайла = Новый Соответствие();
	КонецЕсли;
	...
	ЗначениеКлючаКэша = НастройкиИзФайла.Получить(КлючКэша);
	Если ТипЗнч(ЗначениеКлючаКэша) <> Тип("Соответствие") Тогда
		Если ЗначениеКлючаКэша <> Неопределено Тогда
			Лог.Отладка("Настройка кэша %1 имеет некорректный тип, будет пересоздана", КлючКэша);
		Иначе
			Лог.Отладка("Нет настройки %1 в файле ./build/cache.json", КлючКэша);
		КонецЕсли;
		НастройкиИзФайла.Вставить(КлючКэша, Новый Соответствие);
	КонецЕсли;
