...
Процедура СобратьИзИсходниковТекущуюКонфигурацию(Знач ВходнойКаталог,
	...
	Если ОбновитьФайлВерсий Тогда
		ПутьКФайлуСостояния = ОбъединитьПути(ВходнойКаталог, "ConfigDumpInfo.xml");
		УправлениеИБ.ВыгрузитьВФайлСостояниеКонфигурации(ПутьКФайлуСостояния, ИмяРасширения);
	КонецЕсли;
