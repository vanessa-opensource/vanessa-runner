...
//   ИспользуетсяВРаспределеннойИнформационнойБазе - Булево, Неопределено - расширение используется в РИБ
//   ОбластьДействия - Булево, Неопределено - если Истина - область действия ИБ, иначе - разделитель данных
//
Процедура ИзменитьСвойстваРасширения(Знач ИмяРасширения, Знач Активность, 
	Знач БезопасныйРежим, Знач ИмяПрофиляБезопасности, Знач ЗащитаОтОпасныхДействий,
	Знач ИспользуетсяВРаспределеннойИнформационнойБазе, Знач ОбластьДействия) Экспорт

	Лог.Информация("Выполняю установку свойств расширения %1", ИмяРасширения);

	УправлениеИБ.ИзменитьСвойстваРасширения(ИмяРасширения, Активность, 
	...
	Лог.Информация("Установка свойств расширения %1 завершена", ИмяРасширения);

КонецПроцедуры

// Выводит в лог информацию о расширении конфигурации
//
// Параметры:
//   ИмяРасширения - Строка - Имя расширения, у которого будем изменять свойства
//
Процедура ИнформацияОРасширении(Знач ИмяРасширения) Экспорт

	ИнформацияОРасширении = УправлениеИБ.ИнформацияОРасширении(ИмяРасширения);
	Лог.Информация("Информация о расширении конфигурации:%2%1", ИнформацияОРасширении, Символы.ПС);
