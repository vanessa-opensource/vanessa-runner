...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	ВыполнитьПроверкуEDT(РабочаяОбласть, ИмяФайлаРезультата, СписокПапокСПроектами, СписокИменПроектов,
	...
	Лог.Отладка("КоличествоПроверок %1", РезультатТестирования.КоличествоПроверок);
	Лог.Отладка("КоличествоПропущено %1", РезультатТестирования.КоличествоПропущено);
	Лог.Отладка("КоличествоУпало %1", РезультатТестирования.КоличествоУпало);
	Лог.Отладка("Ошибки %1", РезультатТестирования.Ошибки.Количество());
	Лог.Отладка("ВсеОшибки:
	| %1", РезультатТестирования.ВсеОшибки);

	Успешно = (РезультатТестирования.Ошибки.Количество() = 0);
	...
	Возврат ?(Успешно, РезультатыКоманд.Успех, РезультатыКоманд.Ошибка);

КонецФункции

///////////////////////////////////////////////////////////////////////////////////////////////////

Процедура ВыполнитьПроверкуEDT(Знач РабочаяОбласть, ИмяФайлаРезультата, СписокПапокСПроектами, СписокИменПроектов,
							Знач ВерсияEDT, ВсегдаИспользоватьПоследнююВерсиюEDT)

	Если Не ЗначениеЗаполнено(РабочаяОбласть) Тогда

		ВызватьИсключение "Рабочая область (--workspace-location) не указана. Проверка проекта пропущена.";
	...
	Если Не ЗначениеЗаполнено(СписокПапокСПроектами)
		...
		ВызватьИсключение "Проекты к проверке (--project-list или project-name-list) не указаны. Проверка проекта пропущена.";
	...
	Если КодВозврата <> 0 Тогда
		...
		ВызватьИсключение СтрШаблон("Возникла ошибка: код возврата %1,
