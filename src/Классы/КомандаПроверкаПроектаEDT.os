...
Перем кэшСоответствиеМетаданныхКаталогам;
...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	"Необязательный аргумент. Путь к файлам проекта. Если заполнен, то в отчетах аллюр будут ссылки на конкретные строки с ошибками.
	| Например, --project-url https://github.com/1C-Company/GitConverter/tree/master/GitConverter/src ");
...
Функция ПолучитьСсылкуНаСтроку( Знач пМетаданные, Знач пСтрока )
	...
	Если компоненты.Количество() > 2 Тогда
		...
		Если ВРег( компоненты[2] ) = "МОДУЛЬОБЪЕКТА" Тогда
			...
			компонентыСсылки.Добавить( "ObjectModule.bsl" );
			
		ИначеЕсли ВРег( компоненты[2] ) = "МОДУЛЬ" Тогда
			
			компонентыСсылки.Добавить( "Module.bsl" );
			
		ИначеЕсли ВРег( компоненты[2] ) = "МОДУЛЬМЕНЕДЖЕРА" Тогда
			
			компонентыСсылки.Добавить( "ManagerModule.bsl" );
			
		ИначеЕсли ВРег( компоненты[2] ) = "МОДУЛЬНАБОРАЗАПИСЕЙ" Тогда
			
			компонентыСсылки.Добавить( "RecordSetModule.bsl" );
			
		ИначеЕсли ВРег( компоненты[2] ) = "ФОРМА" Тогда
		...
		ИначеЕсли ВРег( компоненты[2] ) = "КОМАНДА" Тогда
...
Функция СоответствиеМетаданнымКаталогам() Экспорт
