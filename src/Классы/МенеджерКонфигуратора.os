...
Процедура ЗапуститьВРежимеПредприятияСПроверкойВыполнения(
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ПутьКФайлуСтатусаВыполнения) Тогда
			...
			Если СокрЛП(Результат) <> "0" И СокрЛП(Результат) <> "2" Тогда
				ВызватьИсключение "Результат работы не равен 0 или 2, а равен "+ Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	Исключение
		...
		Если ФС.Существует(ПутьДамп) Тогда
			...
			КонецЕсли;
		Иначе
			Лог.Информация("Не существует файл вывода от 1С - %1", ПутьДамп);
		КонецЕсли;
...
Процедура ПодключитьсяКХранилищу(
			Знач ПутьХранилища, Знач Логин, Знач Пароль,
			Знач ИгнорироватьНаличиеПодключеннойБД = Ложь, Знач ЗаменитьКонфигурациюБД = Истина,
			Знач ИмяРасширения = "") Экспорт
...
Процедура РазложитьМодули1СПоПапкамСогласноИерархииМетаданных(Знач КаталогПлоскойВыгрузки, 
	Знач КаталогИерархическойВыгрузки, Знач Формат)
