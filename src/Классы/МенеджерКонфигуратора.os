...
Процедура Инициализация(Знач ПараметрыКоманды, Знач СтрокаПодключения, Знач Пользователь = "", Знач Пароль = "",
	...
	Если НЕ ПустаяСтрока(ВерсияПлатформы) Тогда
		...
		КонецЕсли;
		ЗаданнаяВерсияПлатформы = ВерсияПлатформы;
...
// Установить контекст
// Метод устанавливает контекст управления конфигуратором по переданным данным подключения
//
// Параметры:
...
Процедура УстановитьКонтекст(ДанныеПодключения) Экспорт
	УправлениеКонфигуратором.УстановитьКонтекст(ДанныеПодключения.ПутьБазы,
...
Функция ПолучитьИзмененияКонфигурации(Знач КаталогВыгрузки,
	...
	Конфигуратор = УправлениеКонфигуратором();
	Конфигуратор.ВыгрузитьИзмененияКонфигурацииВФайл(КаталогВыгрузки,
		ПутьКФайлуИзменений,
...
Функция ТекущаяВерсияПлатформы()
	Если ЗначениеЗаполнено(ЗаданнаяВерсияПлатформы) Тогда
		Возврат ЗаданнаяВерсияПлатформы;
	КонецЕсли;
	...
	Если ВсеСовпадения.Количество() = 0 Тогда
		ВызватьИсключение "Неопределена версия платформы! Путь: " + ПутьПредприятия;
	КонецЕсли;
