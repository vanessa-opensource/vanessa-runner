...
Процедура ЗапуститьВРежимеПредприятия(Знач ПараметрЗапуска="",
	...
	Если Не ОжидатьЗавершения Тогда
		...
		УправлениеКонфигуратором.УстановитьИмяФайлаСообщенийПлатформы(МенеджерВременныхФайлов.НовоеИмяФайла());
...
Процедура ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	Попытка
		ЗапуститьПроцессВанессы(Приложение, Истина, ПутьЛогаВыполненияСценариев );
...
Процедура ЗапуститьПроцессВанессы(Знач СтрокаЗапуска, Знач ОжидатьЗавершения, Знач ПутьКФайлуЛога)
	...
	Если ОжидатьЗавершения Тогда // TODO удалить ненужный параметр ОжидатьЗавершения
