...
Процедура ЗапуститьКонфигуратор(Знач ДопСообщения, Знач ДополнительныеКлючиЗапуска, Знач ОжидатьЗавершения) Экспорт
	...
	ПредставлениеКоманды = ДопСообщения.Ключ;
...
//   ИмяРасширения - <Тип.Вид> - <описание параметра>
...
Процедура ЗапуститьОбновлениеИзХранилища(
			...
			Знач ВерсияХранилища = "", Знач ИмяРасширения = "", Знач ДополнительныеКлючиЗапуска = "") Экспорт
	...
	Если Не ПустаяСтрока(ИмяРасширения) Тогда
		...
		ТекстНачалаОперации		= "Выполняю обновление расширения " + ИмяРасширения + " из хранилища";
		ТекстОкончанияОперации	= "Обновление расширения из хранилища завершено";
	Иначе
		ТекстНачалаОперации		= "Выполняю обновление конфигурации из хранилища";
		ТекстОкончанияОперации	= "Обновление конфигурации из хранилища завершено";
	КонецЕсли;
	Если Не ПустаяСтрока(ДополнительныеКлючиЗапуска) Тогда
		Параметры.Добавить(ДополнительныеКлючиЗапуска);
	КонецЕсли;

	Лог.Информация(ТекстНачалаОперации);
	...
	Попытка
		...
		Лог.Информация(ТекстОкончанияОперации);
	Исключение
		Лог.Ошибка(УправлениеКонфигуратором.ВыводКоманды());
		ВызватьИсключение ТекущаяПроцедура;
	КонецПопытки;
...
//
Процедура СоздатьХранилище(Знач ПутьХранилища, Знач ЛогинАдминистратора, Знач ПарольАдминистратора,
		Знач ИмяРасширения = "") Экспорт
...
//   ЛогинАдминистратора - <Строка> - 
//   ПарольАдминистратора - <Строка> - 
//   ИгнорироватьНаличиеПодключеннойБД - <Булево> - необязательно. По умолчанию Ложь
//   ЗаменитьКонфигурациюБД - <Булево> - необязательно. По умолчанию Истина
//   ИмяРасширения - <Строка> - необязательно. По умолчанию пустая строка
//
Процедура ПодключитьсяКХранилищу(Знач ПутьХранилища, Знач Логин, Знач Пароль,
	...
	Параметры.Добавить("/ConfigurationRepositoryF """ +ПутьХранилища + """");
	Параметры.Добавить("/ConfigurationRepositoryN """ +Логин + """");

	Если Не ПустаяСтрока(Пароль) Тогда
		Параметры.Добавить("/ConfigurationRepositoryP """ +Пароль + """");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ИмяРасширения) Тогда
		Параметры.Добавить("-Extension """ +ИмяРасширения + """");
		ТекстНачалаОперации		= "Выполняю подключение к хранилищу расширения " + ИмяРасширения;
		ТекстОкончанияОперации	= "Подключение к хранилищу расширения завершено";
	Иначе
		ТекстНачалаОперации		= "Выполняю подключение к хранилищу конфигурации";
		ТекстОкончанияОперации	= "Подключение к хранилищу конфигурации завершено";
	КонецЕсли;
...
// Параметры:
//	ИмяРасширения - <Строка> - необязательно. По умолчанию пустая строка
//	ДополнительныеКлючиЗапуска - Строка - необязательно. По умолчанию пустая строка
//
Процедура ОтключитьсяОтХранилища(Знач ИмяРасширения = "", Знач ДополнительныеКлючиЗапуска = "") Экспорт
	...
	Если Не ПустаяСтрока(ИмяРасширения) Тогда
		Параметры.Добавить("-Extension """ +ИмяРасширения + """");
		ТекстНачалаОперации		= "Выполняю отключение от хранилища расширения " + ИмяРасширения;
		ТекстОкончанияОперации	= "Отключение расширения от хранилища завершено";
	Иначе
		ТекстНачалаОперации		= "Выполняю отключение от хранилища конфигурации";
		ТекстОкончанияОперации	= "Отключение конфигурации от хранилища завершено";
	КонецЕсли;
...
Процедура СоздатьПользователяХранилища(Знач ПутьХранилища, Знач Логин, Знач Пароль,
								...
								Знач РольПользователя,
								Знач ИмяРасширения = "") Экспорт

	Если ЗначениеЗаполнено(ИмяРасширения) Тогда
		Лог.Информация("Выполняю создание пользователя хранилища расширения " + ИмяРасширения);
	Иначе
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ИмяРасширения) Тогда
			Лог.Информация("Создание пользователя хранилища расширения " + ИмяРасширения + " завершено");
		Иначе
...
Процедура СохранитьВерсиюХранилищаВФайл(Знач ПутьХранилища, Знач ЛогинАдминистратора, Знач ПарольАдминистратора,
				Знач ВерсияХранилища, Знач ПутьКФайлу, Знач ИмяРасширения = "") Экспорт
...
//   Логин          - Строка - <описание параметра>
//   Пароль         - Строка - <описание параметра>
//   СписокОбъектов - Строка - Строка путь к файлу xml с содержанием в формате http://its.1c.ru/db/v839doc#bookmark:adm:TI000000712
...
Процедура ПоместитьИзмененияОбъектовВХранилище(Знач ПутьХранилища, Знач Логин, Знач Пароль,
	...
	Попытка
		...
		ХранилищеКонфигурации.ПоместитьИзмененияОбъектовВХранилище(
...
Процедура ВыгрузитьКонфигурациюВИсходники(Знач ФайлКонфигурации, Знач ВыходнойКаталог,
	...
	ТекущаяПроцедура = "ВыгрузитьКонфигурациюВИсходники";
