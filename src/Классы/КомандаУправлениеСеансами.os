...
Функция ПолучитьСписокСеансов()
	...
	Для Каждого ТекПроцесс Из Соединения Цикл
		...
		Если ПроверитьФильтр Тогда
			Если СеансНеПодходитПодФильтр(ТекПроцесс["app-id"], ТекПроцесс["user-name"]) Тогда
				Продолжить;
			КонецЕсли;
...
Функция РазобратьПоток(Знач Поток)
...
Процедура ДополнитьФильтрСеансов(ФильтрыСеансовПользователь, ФильтрСеансовВстроенный)
	...
	Для Каждого Фильтр Из ФильтрСеансовВстроенный Цикл
		...
		Если ФильтрыСеансовПользователь.Свойство(Фильтр.Ключ) Тогда
			...
			//TODO в таком варианте встроенные дефолт-настройки нельзя будет переопределить, что нехорошо!
...
Функция СеансНеПодходитПодФильтр(Приложение, Пользователь)
	...
	Возврат (СеансВключенВФильтр И мНастройки.ФильтрКакИсключение)
		ИЛИ (НЕ СеансВключенВФильтр И НЕ мНастройки.ФильтрКакИсключение);
