...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Если мЭтоУправлениеСеансами Тогда
		...
		Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды,
			...
			|DEFAULT - Все фильтры, включенные по-умолчанию (из настроек ванесса-раннер) считаются включенными, остальные - игнорируются.
			|EXCEPT - Все фильтры, кроме указанных, считаются включенными. Режим пропуска сеансов по фильтру.
			|* ONLY - Только указанные фильтры считаются включенными.
			|ALL - Все фильтры считаются включенными.
			|При незаполненной настройке управления применяется режим ONLY. 
...
Процедура ПрочитатьПараметры(Знач ПараметрыКоманды)
	...
	мНастройки.Вставить("ФильтрКакИсключение", Ложь);

	Если ЗначениеЗаполнено(ПараметрыКоманды["--mode"]) Тогда

		РежимРаботыФильтра = ВРег(ПараметрыКоманды["--mode"]);
		мНастройки.Вставить("ФильтрСеансов", ПолучитьИзмененныйФильтр(РежимРаботыФильтра, мНастройки.ФильтрСеансов));

		Если РежимРаботыФильтра = "EXCEPT" Тогда
			мНастройки.Вставить("ФильтрКакИсключение", Истина);
		КонецЕсли;
...
Функция ПолучитьСписокСеансов()
	...
	Соединения = РазобратьПоток(ЗапуститьПроцесс(КомандаЗапуска));

	ПроверитьФильтр = ЗначениеЗаполнено(мНастройки.ФильтрСеансов);

	Для Каждого ТекПроцесс Из Соединения Цикл

		Если ПроверитьФильтр Тогда
			СеансВключенВФильтр = СеансВФильтре(Новый Структура("Приложение, Пользователь", ТекПроцесс["app-id"], ТекПроцесс["user-name"]));

			Если (СеансВключенВФильтр И мНастройки.ФильтрКакИсключение)
			 ИЛИ (НЕ СеансВключенВФильтр И НЕ мНастройки.ФильтрКакИсключение) Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПолучитьСоединенияРабочегоПроцесса(Знач РабочийПроцесс)
	...
	Для Каждого ТекПроцесс Из Соединения Цикл
		Если ВРег(ТекПроцесс["app-id"]) = "RAS" Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьИзмененныйФильтр(Знач РежимРаботыФильтра, Знач ФильтрСеансов)
	...
	СтрокаФильтраИзНастроекВанессаРаннер = ""; // Тут надо получить фильтр по-умолчанию из настроек ванесса-раннер

	Если РежимРаботыФильтра = "OFF" Тогда
		Возврат Новый Структура;
	
	ИначеЕсли РежимРаботыФильтра = "DEFAULT" Тогда
		Возврат ПолучитьСоставляющиеФильтра(СтрокаФильтраИзНастроекВанессаРаннер);

	ИначеЕсли РежимРаботыФильтра = "ALL" Тогда
		ФильтрСеансовВанессаРаннер = ПолучитьСоставляющиеФильтра(СтрокаФильтраИзНастроекВанессаРаннер);
		ДополнитьФильтрСеансов(ФильтрСеансов, ФильтрСеансовВанессаРаннер);
	КонецЕсли;
...
Процедура ДополнитьФильтрСеансов(ФильтрыСеансовПользователь, ФильтрыСеансовВанесса) Экспорт
	
	Для Каждого Фильтр Из ФильтрыСеансовВанесса Цикл

		МассивКлючаВанесса = Фильтр.Значение;
		...
		Если ФильтрыСеансовПользователь.Свойство(Фильтр.Ключ) Тогда
			...
			ДополнитьМассивКлюча(МассивКлючаВанесса, МассивКлючаПользователь);
		КонецЕсли;

		ФильтрыСеансовПользователь.Вставить(Фильтр.Ключ, МассивКлючаВанесса);
...
Функция СеансВФильтре(Сеанс)
	...
	Результат = ПараметрСеансаВФильтре("appid", Сеанс.Приложение);
	Если Не Результат Тогда
		Результат = ПараметрСеансаВФильтре("name", Сеанс.Пользователь);
	КонецЕсли;
...
Функция ПараметрСеансаВФильтре(ИмяФильтра, ПроверяемоеЗначение)
	...
	Если Не мНастройки.ФильтрСеансов.Свойство(ИмяФильтра) Тогда
		Возврат Ложь;
	КонецЕсли;
