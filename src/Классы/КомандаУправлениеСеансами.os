...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Если мЭтоУправлениеСеансами Тогда
		...
		Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды,
			"--is-exclude-filter",
			"Фильтр поиска сеансов переключен в режим исключения (y/n). Применяется для --filter с перечисленными вариантами.
			|При включенном флаге фильтр работает в режиме 'пропустить перечисленные' и сеансы из фильтра не будут завершены или учтены.
			|Пример: ... kill -filter appid=Designer --is-exclude-filter...");
...
Процедура ПрочитатьПараметры(Знач ПараметрыКоманды)
	...
	мНастройки.Вставить("ФильтрКакИсключение", ПараметрыКоманды["--is-exclude-filter"]);
