...
Функция ПолучитьСписокСеансов()
	...
	Для Каждого Элемент Из Соединения Цикл
		...
		Если СеансВФильтре(Новый Структура("Приложение, Пользователь", Элемент["app-id"], Элемент["user-name"])) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьСоединенияРабочегоПроцесса(Знач РабочийПроцесс)
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Если СтрНайти(ВРег(ТекстОшибки), ВРег("Рабочий процесс с указанным идентификатором не найден")) = 0 Тогда
			ВызватьИсключение ОписаниеОшибки();
		Иначе
...
Функция РазорватьСоединениеСПроцессом(Знач РабочийПроцесс, Знач Соединение)
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Если СтрНайти(ВРег(ТекстОшибки), ВРег("Соединение")) = 0 И СтрНайти(ВРег(ТекстОшибки), ВРег("не найдено")) = 0 Тогда
			ВызватьИсключение ОписаниеОшибки();
		КонецЕсли;
		Результат = Ложь;
	КонецПопытки;
