...
Процедура ИнициализироватьБазуДанных(РежимыРеструктуризации, Знач ПутьКSRC, Знач ПутьКDT, Знач ДанныеПодключения,
	...
	ОперацияПоддерживаетсяIbcmd = ЗначениеЗаполнено(ПутьКSRC)
		И (НЕ ПараметрыХранилища.РежимОбновления);
	...
	ЗагрузкаИзSrc(МенеджерСборки, ПутьКSRC); 
...
Процедура ЗагрузкаИзSrc(МенеджерСборки, ПутьКSRC) 

	Если ПустаяСтрока(ПутьКSRC) Тогда
		Возврат;
	КонецЕсли;
	
	Лог.Информация("Запускаю загрузку конфигурации из исходников");
	ПолныйПутьКSRC = Новый Файл(ОбъединитьПути(КорневойПутьПроекта, ПутьКSRC)).ПолноеИмя;
	СписокФайлов = "";
	МенеджерСборки.СобратьИзИсходниковТекущуюКонфигурацию(ПолныйПутьКSRC, СписокФайлов, Ложь);
