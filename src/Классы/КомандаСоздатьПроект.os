...
Процедура ИнициализацияПроекта()
	...
	Лог.Информация("Начат процесс создания проекта");
	
	Если Настройки.ИспользоватьШаблон Тогда
		
		Результат = КлонироватьШаблон();
		Если НЕ Результат Тогда
			Лог.Информация("Не удалось клонировать шаблон");
			Возврат;
		КонецЕсли;
		
		Пакеты = ПакетыШаблона();
...
Функция КлонироватьШаблон()
	...
	Лог.Информация("Шаблон для установки: %1", Шаблон);
	...
	Результат = ВыполнитьКомандуПоСтрокеЗапуска("git", ПараметрыКоманды);
	
	Возврат Результат;
	
КонецФункции

Функция ПакетыШаблона()
...
Функция ПакетыПоУмолчанию()
	...
	ПакетГитсинк = ПакетДляДобавления("gitsync", МассивКоманд);
...
Процедура ВыполнитьКомандыУстановкиПакета(Команды)
	...
	Для Каждого СтрокаКоманды Из Команды Цикл
		...
		Отказ = ВыполнитьКомандуПоСтрокеЗапуска(СтрокаКоманды);
...
Функция ВыполнитьКомандуПоСтрокеЗапуска(СтрокаЗапуска, Параметры = Неопределено)
	...
	Иначе
		...
		Для Каждого СтрокаПараметр Из Параметры Цикл
			Команда.ДобавитьПараметр(СтрокаПараметр);
		КонецЦикла;
