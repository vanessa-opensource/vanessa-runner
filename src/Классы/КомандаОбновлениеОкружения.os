...
Функция СоздатьДанныеПодключения(ДанныеПодключения)
	...
	// Дальше грязный хак, потому что в 2.0.0 есть баг. Как исправят, надо вернуть назад https://github.com/EvilBeaver/OneScript/issues/1646
	// ЗаполнитьЗначенияСвойств(_ДанныеПодключения, ДанныеПодключения);
	Для Каждого КлючИЗначение Из ДанныеПодключения Цикл
			_ДанныеПодключения.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	// Конец грязного хака
