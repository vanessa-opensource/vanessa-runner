...
Процедура ПроверитьНеобходимостьЗавершенияПрограммы() Экспорт
	...
	Для каждого Окн Из Окна Цикл
		Если ТипЗнч(Окн) = Тип("ОкноКлиентскогоПриложения") Тогда
			...
			// проверка наличия модальных сообщений
			ЕстьФормаМесседжБокс = Ложь;
			Для Каждого ФормаОкна из Окн.Содержимое Цикл
				Если ТипЗнч(ФормаОкна) = Тип("ФормаКлиентскогоПриложения") Тогда
					Если ФормаОкна.ИмяФормы = "MessageBox" Тогда
						ЕстьФормаМесседжБокс = Истина;
						ЭлементСТекстомСообщения = ФормаОкна.Элементы.Найти("Message");
						Если ЭлементСТекстомСообщения = Неопределено Тогда
							ТекстСообщения = "Возникла неопределенная ошибка";
						Иначе
							ТекстСообщения = ЭлементСТекстомСообщения.Заголовок;
						КонецЕсли;
						Сообщить("ERROR: " + ТекущаяДата() + " " + ТекстСообщения);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			Если ЕстьФормаМесседжБокс Тогда
				СчетчикОжиданияРезультатов = МаксИтерацийОжиданияРезультатов + 1;
				МожноЗавершатьРаботу = Истина;
				ОшибкаОбновления = Истина;
				Прервать;
