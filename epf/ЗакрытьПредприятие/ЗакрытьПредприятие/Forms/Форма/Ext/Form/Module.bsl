...
Процедура ПриОткрытии(Отказ)
	...
	Для каждого Стр из ПараметрыЗапуска Цикл
		Если СтрНайти(Стр, СтрокаПоиска) > 0 Тогда
			ПутьКФайлуУспеха = СтрЗаменить(Стр, СтрокаПоиска, "");
			Сообщить(ПутьКФайлуУспеха);
			ЗаписатьФайлУспеха(Ложь);
			Прервать;
		КонецЕсли;	
	КонецЦикла;	 	
...
&НаКлиенте
Процедура ЗаписатьФайлУспеха(Записываем = Истина)
	
	Если ПутьКФайлуУспеха = "" Тогда
		Возврат;
	КонецЕсли; 
	
	Если Записываем И НЕ ОшибкаОбновления Тогда
		
		ТекстовыйФайл = Новый ТекстовыйДокумент;
		ТекстовыйФайл.УстановитьТекст("0");
		ТекстовыйФайл.Записать(ПутьКФайлуУспеха, КодировкаТекста.UTF8); 
		
	Иначе
		
		Попытка
			УдалитьФайлы(ПутьКФайлуУспеха);
		Исключение
			//ОписаниеОшибки()
		КонецПопытки;
